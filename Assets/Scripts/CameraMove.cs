...
public class CameraMove : MonoBehaviour {
    ...
    void Start()
    {
        offset = new Vector2(Mathf.Abs(offset.x * 3), offset.y);
        FindPlayer();
    ...
    void LateUpdate()
    {
        if (_target)
        {
            ...
            if (faceLeft)
            {
                target = new Vector3(_target.position.x - offset.x / 2, _target.position.y + offset.y + dynamicSpeed, transform.position.z);
            }
            else
            {
                target = new Vector3(_target.position.x + offset.x / 2, _target.position.y + offset.y + dynamicSpeed, transform.position.z);
            }
            Vector3 currentPosition = Vector3.Lerp(transform.position, target, damping * Time.deltaTime);// * 1.2F);
